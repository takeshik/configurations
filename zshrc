################################################################################
###
### .zshrc - rc script for zsh
### Written by takeshik as root
###
################################################################################

## Set options
setopt \
	always_last_prompt \
	append_history \
	auto_cd \
	auto_list \
	auto_menu \
	auto_param_keys \
	auto_param_slash \
	auto_pushd \
	complete_in_word \
	extended_glob \
	extended_history \
	hashall \
	hist_expand \
	hist_ignore_dups \
	hist_no_store \
	hist_reduce_blanks \
	inc_append_history \
	list_ambiguous \
	list_packed \
	magic_equal_subst \
	mark_dirs \
	no_list_beep \
	notify \
	numeric_glob_sort \
	prompt_subst \
	pushd_minus \
	rmstar_wait \
	share_history \
;

## Prompt
export PROMPT='[%B%{%(?.${fg[yellow]}.${fg[magenta]})%}%h%b%{${reset_color}%}|%B%U%{${fg[red]}%}%n@%M%u%b%{${reset_color}%} %B%{${fg[blue]}%}%d%b%{${reset_color}%}]%(1j.%B%{${fg[green]}%}%#%b%{${reset_color}%}.%#) ';
export RPROMPT='(r:%? j:%j%)'
export PROMPT2='%B%{${fg[black]}%}--->%b%{${reset_color}%} '
export RPROMPT2='%_'

export PATH=/root/sbin:/root/bin:$PATH

HISTFILE=~/.zsh_history
HISTSIZE=99999999
SAVEHIST=99999999
WORDCHARS='*?_-.[]~=&;!#$%^(){}<>'

## Autoload
autoload -U colors; colors
autoload -U compinit; compinit
autoload    zed

## Watch
watch=(all all)
watch="all"

## Keybind
bindkey "^?"    backward-delete-char
bindkey "^H"    backward-delete-char
bindkey "^[[3~" delete-char
bindkey "^[[1~" beginning-of-line
bindkey "^[[4~" end-of-line

## Visual styles
zstyle ':completion:*' \
	list-colors \
		'no=00' \
		'fi=00' \
		'di=01;34' \
		'ln=01;36' \
		'pi=40;33' \
		'so=01;35' \
		'do=01;35' \
		'bd=40;33;01' \
		'cd=40;33;01' \
		'or=01;05;37;41' \
		'mi=01;05;37;41' \
		'su=37;41' \
		'sg=30;43' \
		'tw=30;42' \
		'ow=34;42' \
		'st=37;44' \
		'ex=01;32' \
		'*.tar=01;31' \
		'*.tgz=01;31' \
		'*.svgz=01;31' \
		'*.arj=01;31' \
		'*.taz=01;31' \
		'*.lzh=01;31' \
		'*.lzma=01;31' \
		'*.zip=01;31' \
		'*.z=01;31' \
		'*.Z=01;31' \
		'*.dz=01;31' \
		'*.gz=01;31' \
		'*.bz2=01;31' \
		'*.bz=01;31' \
		'*.tbz2=01;31' \
		'*.tz=01;31' \
		'*.deb=01;31' \
		'*.rpm=01;31' \
		'*.jar=01;31' \
		'*.rar=01;31' \
		'*.ace=01;31' \
		'*.zoo=01;31' \
		'*.cpio=01;31' \
		'*.7z=01;31' \
		'*.rz=01;31' \
		'*.jpg=01;35' \
		'*.jpeg=01;35' \
		'*.gif=01;35' \
		'*.bmp=01;35' \
		'*.pbm=01;35' \
		'*.pgm=01;35' \
		'*.ppm=01;35' \
		'*.tga=01;35' \
		'*.xbm=01;35' \
		'*.xpm=01;35' \
		'*.tif=01;35' \
		'*.tiff=01;35' \
		'*.png=01;35' \
		'*.svg=01;35' \
		'*.mng=01;35' \
		'*.pcx=01;35' \
		'*.mov=01;35' \
		'*.mpg=01;35' \
		'*.mpeg=01;35' \
		'*.m2v=01;35' \
		'*.mkv=01;35' \
		'*.ogm=01;35' \
		'*.mp4=01;35' \
		'*.m4v=01;35' \
		'*.mp4v=01;35' \
		'*.vob=01;35' \
		'*.qt=01;35' \
		'*.nuv=01;35' \
		'*.wmv=01;35' \
		'*.asf=01;35' \
		'*.rm=01;35' \
		'*.rmvb=01;35' \
		'*.flc=01;35' \
		'*.avi=01;35' \
		'*.fli=01;35' \
		'*.gl=01;35' \
		'*.dl=01;35' \
		'*.xcf=01;35' \
		'*.xwd=01;35' \
		'*.yuv=01;35' \
		'*.pdf=00;32' \
		'*.ps=00;32' \
		'*.txt=00;32' \
		'*.patch=00;32' \
		'*.diff=00;32' \
		'*.log=00;32' \
		'*.tex=00;32' \
		'*.doc=00;32' \
		'*.aac=00;36' \
		'*.au=00;36' \
		'*.flac=00;36' \
		'*.mid=00;36' \
		'*.midi=00;36' \
		'*.mka=00;36' \
		'*.mp3=00;36' \
		'*.mpc=00;36' \
		'*.ogg=00;36' \
		'*.ra=00;36' \
		'*.wav=00;36' \
;

## Alias
alias -g ls='ls -av --color=auto'
alias -g ls!='ls -R'
alias -g ll='ls --format=verbose'
alias -g ll!='lsx -R'
alias -g rm='rm -v'
alias -g rm!='rm -R'
alias -g rm!!='rm! -f'
alias -g cp='cp -av'
alias -g cp!='cp -f'
alias -g ln='ln -v'
alias -g ln!='ln -f'
alias -g mv='mv -v'
alias -g mv!='mv -f'
alias -g mkdir='mkdir -pv'
alias -g md='mkdir'
alias -g chown='chown -cv'
alias -g chown!='chown -R'
alias -g chgrp='chgrp -cv'
alias -g chgrp!='chgrp -R'
alias -g chmod='chmod -cv'
alias -g chmod!='chmod -R'
alias -g chacl!='chacl -r'
alias -g getfacl='getfacl --tabular'
alias -g getfacl!='getfacl -LR'
alias -g setfacl!='setfacl -LR'
alias -g pwd='pwd -P'
alias -g grep='grep -n --color --mmap'
alias -g history='history -Ddi'
alias -g ps='ps -AHo pid,user,group,%cpu,%mem,nlwp,vsize,rssize,tname,stat,ni,start_time,cputime,args'
alias -g ja='LANG=ja_JP.UTF8 LC_ALL=ja_JP.UTF-8'
alias -g en='LANG=en_US.UTF8 LC_ALL=en_US.UTF-8'
alias -g h='history 0'
alias -g g='grep'
alias -g s='dirs'
alias -g l='lv'
alias -g k='kill'
alias -g K='kill -KILL'
alias -g ka='killall'
alias -g KA='killall -KILL'
alias -g r='popd'
alias -g j='jobs'
alias -g H='head'
alias -g T='tail'
alias -g TF='tail -F'
